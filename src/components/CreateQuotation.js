...
function CreateQuotation() {
    ...
    getQuotationSettings()
      .then((settings) => {
        const businessType = String(
          globalSettings?.business_type || "GENERAL",
        ).toUpperCase();
        const nextMode =
        ...
        setGstPricingMode(
          settings?.gst_pricing_mode ||
            globalSettings?.gst_pricing_mode ||
            "EXCLUSIVE",
        );
  ...
  const handleSubmit = async () => {
    ...
    const validItems = items.filter(
      (i) =>
        i.product?.id && Number(i.quantity) > 0 && Number(i.selling_price) >= 0,
    );

    if (validItems.length !== items.length) {
      return showNotification(
        "Please check item quantities and prices",
        "warning",
      );
    }
          ...
          <div className="quotation-tabs">
            <button
              ...
              className={`quotation-tab ${
                activeQuotationTab === "items" ? "quotation-tab--active" : ""
              }`}
            ...
            <button
              type="button"
              className={`quotation-tab ${
                activeQuotationTab === "location" ? "quotation-tab--active" : ""
              }`}
